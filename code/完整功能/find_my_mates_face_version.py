...
class find_my_mate():
    ...
    def Led_Tablet_init(self):
        ...
        print (Fore.GREEN + '[I]: 平板与LED初始化成功')
    ...
    def callback_sound_det(self, msg):
        if self.if_need_record:
            print (Fore.GREEN + '[I]: 检测到声音(位于回调函数中) ')
            ox = 0
            ...
            print (Fore.GREEN + '[I]: 检测到声音，现在未进行录音动作 ')
    ...
    def analyze_content(self):
        ...
        for i in range(len(self.name_list)):
            if re.search(self.name_list[i].lower(), self.recog_result) != None:
                print (Fore.GREEN + ('[I]: 找到的人：' + self.name_list[i].lower()))
                person_found = True
                # 记下当前人的名字
